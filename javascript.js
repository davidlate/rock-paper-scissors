...
function game(selection) {
  ...
  if (playerScore == 5)
    message = "You beat the computer! Press Enter to reset the game";
  if (computerScore == 5)
    message = "The computer won this game.  Press Enter to try again";
...
function getSelectionFromInput(e) {
  ...
  if (e.type == "keydown") {
    // if input is a keydown

    const div = document.querySelector(`.game-button[data-key="${e.key}"]`);
    if (!div) return;
    selection = div.id;
  } else if (e.type == "click") {
    // if input is a click
    if (e.target.classList.contains("keypress"))
      selection = e.target.parentNode.id;
...
function setDefaultMessages() {
  ...
  if (gameNumber == 0)
    msgBox.textContent = `Play against the computer.  First to ${MAX_SCORE} wins!`;
...
function checkWin(computerSelection) {
  if (gameState !== "playerWin" && gameState !== "computerWin") return;
    ...
    if (gameState == "computerWin" && computerSelection == "none") {
      ...
      msgBox2.textContent = `The computer beat you by ${
        computerScore - playerScore
      } point${plural}. Reset to try again!`;
    }
...
function updateGame(selection) {
  ...
  if (selection === "reset") {
    ...
    if (
      ...
      const scores = [playerScore, computerScore, gameNumber];
...
document.querySelector(
  "#message"
).textContent = `Play against the computer. First to ${MAX_SCORE} wins!`;
